...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        fc_out, fc_cache = x.view(x.shape[0], -1) @ w + b, (x, w, b)
        out, relu_cache = torch.relu(fc_out), fc_out
    ...
    def backward(dout, cache):
        ...
        dfc_out = dout * (relu_cache > 0).float()
        ...
        dw = x.view(x.shape[0], -1).t() @ dfc_out
        db = torch.sum(dfc_out, dim=0)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dh1, grads['W2'], grads['b2'] = Linear.backward(dscores, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
            self.params[f'W{i}'] = weight_scale * torch.randn(dims[i - 1], dims[i], dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            scores, cache = Linear_ReLU.forward(scores, Wi, bi)
            if self.use_dropout:
        ...
        scores, cache_last = Linear.forward(scores, W_last, b_last)
        ...
        for i in range(1, self.num_layers + 1):
            ...
            reg_loss += 0.5 * self.reg * torch.sum(Wi ** 2)
        ...
        for i in reversed(range(1, self.num_layers)):
            cache = caches.pop()
            if self.use_dropout:
                dhidden = Dropout.backward(dhidden, cache[1])
                dhidden, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dhidden, cache[0])
            else:
                dhidden, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dhidden, cache)
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict, device=device, num_epochs=20, batch_size=50, optim_config={'learning_rate' : 3e-2})
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
